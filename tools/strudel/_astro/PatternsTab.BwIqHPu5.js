import{j as a}from"./jsx-runtime.DDS51bD4.js";import{u as C,$ as v,c as w,d as f,e as D,f as B,p as x,g as k,s as G,h as g,j as h,k as E,m as j}from"./useClient.Bn08DQwb.js";import{r as F}from"./index.DIn8BZQ7.js";function S(...s){return s.filter(Boolean).join(" ")}function _({value:s,onChange:n,items:e}){return a.jsx("div",{className:"flex max-w-lg",children:Object.entries(e).map(([o,c],d,b)=>a.jsx("button",{onClick:()=>n(o),className:S("px-2 border-b h-8 whitespace-nowrap",s===o?"border-foreground":"border-transparent"),children:c.toLowerCase()},o))})}const N=["by","url","genre","license"];function R(s){const n=/\/\*([\s\S]*?)\*\/|\/\/(.*)$/gm,e=[...s.matchAll(n)].map(b=>(b[1]||b[2]||"").trim()),o={},[c,d]=(e[0]||"").split('"');c.trim()===""&&d!==void 0&&(o.title=d);for(const b of e){const p=b.split("@").slice(1);for(const l of p){let[i,r]=l.split(/ (.*)/s);if(i=i.trim(),r=(r||"").replaceAll(/ +/g," ").trim(),N.includes(i)){const t=r.split(/[,\n]/).map(m=>m.trim()).filter(m=>m!=="");o[i]=i in o?o[i].concat(t):t}else r=r.replaceAll(/\s+/g," "),o[i]=i in o?o[i]+" "+r:r}}return o}const T=`// Koji Kondo - Swimming (Super Mario World)
stack(
  seq(
    "~",
    "~",
...
.pianoroll({fold:1})`,M=`// Koji Kondo - Princess Zelda's Rescue
stack(
  // melody
...
.pianoroll({fold:1})
`,O=`// adapted from a Barry Harris excercise
"0,2,[7 6]"
  .add("<0 1 2 3 4 5 7 8>")
  .scale('C bebop major')
  .transpose("<0 1 2 1>/8")
...
//.pianoroll({fold:1})`,L=`// "Wavy kalimba"
// @license CC BY-NC-SA 4.0 https://creativecommons.org/licenses/by-nc-sa/4.0/
// @by Felix Roos
...
.clip(.5)
.piano()
.pianoroll()`,J=`// Hirokazu Tanaka - World 1-1
stack(
  // melody
  note(\`<
...
note(
  ...
  .gain(.5)
  .room(.5)
  .lpa(.125).lpenv(-2).v("8:.125").fanchor(.25)`,V=`// "Festival of fingers 3"
// @license CC BY-NC-SA 4.0 https://creativecommons.org/licenses/by-nc-sa/4.0/
// @by Felix Roos
...
n("[-7*3],0,2,6,[8 7]")
  ...
  .slow(2).piano()
//.pianoroll({maxMidi:160})`,Z=`// "Melting submarine"
// @license CC BY-NC-SA 4.0 https://creativecommons.org/licenses/by-nc-sa/4.0/
// @by Felix Roos
...
stack(
    ...
    .delaytime(".16 | .33")
    .delayfeedback(".6 | .8")
  ).sometimes(x=>x.speed("-1"))`,se=`// "Orbit"
// @license CC BY-NC-SA 4.0 https://creativecommons.org/licenses/by-nc-sa/4.0/
// @by Felix Roos
...
stack(
    ...
    .delayfeedback(.7)
    .orbit(2)
  ).sometimes(x=>x.speed("-1"))`,ae=`// "Belldub"
// @license CC BY-NC-SA 4.0 https://creativecommons.org/licenses/by-nc-sa/4.0/
// @by Felix Roos
...
).reset("<x@7 x(5,8,-1)>")`,ie=`// "Jux und tollerei"
// @license CC BY-NC-SA 4.0 https://creativecommons.org/licenses/by-nc-sa/4.0/
// @by Felix Roos
...
.note()
//.pianoroll()
.csound('CoolSynth')`,de=`// "Lounge sponge"
// @license CC BY-NC-SA 4.0 https://creativecommons.org/licenses/by-nc-sa/4.0/
...
`,me=Object.freeze(Object.defineProperty({__proto__:null,amensister:ce,arpoon:be,barryHarris:O,bassFuge:Q,belldub:ae,blippyRhodes:K,caverave:Y,chop:X,csoundDemo:re,delay:ee,dinofunk:oe,echoPiano:z,festivalOfFingers:W,festivalOfFingers3:V,flatrave:le,giantSteps:P,goodTimes:$,holyflute:ne,juxUndTollerei:ie,loungeSponge:de,meltingsubmarine:Z,orbit:se,randomBells:I,sampleDemo:te,sampleDrums:q,sml1:J,swimming:T,undergroundPlumber:H,waa2:U,wavyKalimba:L,zeldasRescue:M},Symbol.toStringTag,{value:"Module"}));Object.fromEntries(Object.entries(me).map(([s,n],e)=>[e,{id:e,code:n,collection:"Stock Examples"}]));const pe=()=>{const s=C(v),n=C(w),e=F.useMemo(()=>{const c=new Map;return c.set(f.featured,s),c.set(f.public,n),c},[s,n]);return{patterns:F.useMemo(()=>Object.assign({},...e.values()),[e]),collections:e}};function he(...s){return s.filter(Boolean).join(" ")}function ue({pattern:s}){const n=F.useMemo(()=>R(s.code),[s]);let e=n.title;if(e==null){const o=new Date(s.created_at);isNaN(o)||(e=o.toLocaleDateString())}return e==null&&(e=s.hash),e==null&&(e="unnamed"),a.jsx(a.Fragment,{children:`${s.id}: ${e} by ${Array.isArray(n.by)?n.by.join(","):"Anonymous"}`})}function ge({showOutline:s,onClick:n,pattern:e,showHiglight:o}){return a.jsx("a",{className:he("mr-4 hover:opacity-50 cursor-pointer block",s&&"outline outline-1",o&&"bg-selection"),onClick:n,children:a.jsx(ue,{pattern:e})})}function y({patterns:s,activePattern:n,onClick:e,started:o}){const c=B(),b=x(c).id;return a.jsx("div",{className:"font-mono text-sm",children:Object.values(s).reverse().map(p=>{const l=p.id;return a.jsx(ge,{pattern:p,showHiglight:l===b,showOutline:l===n&&o,onClick:()=>e(l)},l)})})}function u({children:s,onClick:n,label:e,labelIsHidden:o}){return a.jsxs("button",{className:"hover:opacity-50",onClick:n,title:e,children:[o!==!0&&e,s]})}function ye({context:s}){const n=D(),e=B(),o=x(e),{userPatterns:c,patternFilter:d}=k(),p=pe().collections,l=(t,m=!1)=>{s.handleUpdate(t,m)},i=o?.id,r=!window.parent?.location.pathname.includes("oodles");return a.jsxs("div",{className:"px-4 w-full dark:text-white text-stone-900 space-y-2 pb-4 flex flex-col overflow-hidden max-h-full",children:[a.jsx(_,{value:d,onChange:t=>G.setKey("patternFilter",t),items:g}),d===g.user&&a.jsx("div",{children:a.jsxs("div",{className:"pr-4 space-x-4 border-b border-foreground flex max-w-full overflow-x-auto",children:[a.jsx(u,{label:"new",onClick:()=>{const{data:t}=h.createAndAddToDB();l(t)}}),a.jsx(u,{label:"duplicate",onClick:()=>{const{data:t}=h.duplicate(o);l(t)}}),a.jsx(u,{label:"delete",onClick:()=>{const{data:t}=h.delete(i);l({...t,collection:h.collection})}}),a.jsxs("label",{className:"hover:opacity-50 cursor-pointer",children:[a.jsx("input",{style:{display:"none"},type:"file",multiple:!0,accept:"text/plain,application/json",onChange:t=>E(t.target.files)}),"import"]}),a.jsx(u,{label:"export",onClick:j}),a.jsx(u,{label:"delete-all",onClick:()=>{const{data:t}=h.clearAll();l(t)}})]})}),a.jsxs("section",{className:"flex overflow-y-scroll max-h-full flex-col",children:[d===g.user&&a.jsx(y,{onClick:t=>l({...c[t],collection:h.collection},r),patterns:c,started:s.started,activePattern:n,viewingPatternID:i}),d!==g.user&&Array.from(p.keys()).map(t=>{const m=p.get(t);return a.jsxs("section",{className:"py-2",children:[a.jsx("h2",{className:"text-xl mb-2",children:t}),a.jsx("div",{className:"font-mono text-sm",children:a.jsx(y,{onClick:A=>l({...m[A],collection:t},r),started:s.started,patterns:m,activePattern:n})})]},t)})]})]})}export{_ as B,ue as P,ye as a,S as c};
